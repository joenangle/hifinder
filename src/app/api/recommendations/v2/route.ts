...
interface RecommendationRequest {
  ...
  driverTypeFilter?: string[] // v2 enhancement: driver type filtering
}

// Convert letter grades to numeric scores
function convertGradeToNumeric(grade: string | null | undefined): number | null {
  if (!grade || typeof grade !== 'string') return null

  // Handle corrupted data (text snippets)
  if (grade.length > 3) return null
...
// Calculate expert grade bonus (only for A- or higher)
function calculateExpertGradeBonus(component: RecommendationComponent): number {
  const techScore = convertGradeToNumeric(component.technical_grade)
  const toneScore = convertGradeToNumeric(component.tone_grade)

  if (!techScore && !toneScore) return 0

  // Calculate average grade (handling nulls)
  const scoreCount = (techScore ? 1 : 0) + (toneScore ? 1 : 0)
  const avgGrade = ((techScore || 0) + (toneScore || 0)) / scoreCount

  // Only bonus for grades A- (0.75) or higher
  if (avgGrade >= 0.75) {
    // Max 5% bonus for S-tier components
    return (avgGrade - 0.75) * 0.2
  }
...
function allocateBudgetAcrossComponents(
  ...
  // Updated budget caps for summit-fi gear
  const componentBudgetCaps = {
    headphones: totalBudget * 0.8, // Headphones can take most of the budget
    dac: Math.min(2000, totalBudget * 0.4), // Updated cap for high-end DACs
    amp: Math.min(2000, totalBudget * 0.4), // Updated cap for high-end amps
    combo: Math.min(3000, totalBudget * 0.6) // Updated cap for high-end combos
  }
...
function filterAndScoreComponents(
  ...
  primaryUsage: string,
  maxOptions: number,
  driverTypeFilter?: string[] // v2: optional driver type filter
  ...
  return components
    ...
    .filter(c => {
      // Driver type filtering (v2 enhancement)
      if (driverTypeFilter && driverTypeFilter.length > 0 && c.driver_type) {
        const matchesDriver = driverTypeFilter.some(dt =>
          c.driver_type.toLowerCase().includes(dt.toLowerCase())
        )
      ...
      // Improved price spread filter: ratio-based approach (2.5x max)
      const priceRatio = (c.price_used_max || 0) / (c.price_used_min || 1)
      const hasReasonableRange = priceRatio <= 2.5

      return isAffordable && isInRange && hasReasonableRange
    })
    .sort((a, b) => {
      // Improved price scoring (Option C): prefer items at/under budget
      let aPriceFit: number
      let bPriceFit: number

      if (a.avgPrice <= budget) {
        // Under budget: slight preference for closer to budget
        aPriceFit = 0.95 + (a.avgPrice / budget) * 0.05
      } else {
      ...
      if (b.avgPrice <= budget) {
        // Under budget: slight preference for closer to budget
        bPriceFit = 0.95 + (b.avgPrice / budget) * 0.05
      } else {
        // Over budget: steep penalty
        bPriceFit = Math.max(0, 1 - (b.avgPrice - budget) / budget * 1.5)
      }

      // Value rating interaction: can boost price fit beyond 1.0
      const aValueRating = (a as RecommendationComponent).value_rating
      const bValueRating = (b as RecommendationComponent).value_rating

      if (aValueRating !== null && aValueRating !== undefined) {
        aPriceFit += aValueRating * 0.04 // 4% per point, up to 12% for value_rating=3
      }

      if (bValueRating !== null && bValueRating !== undefined) {
        bPriceFit += bValueRating * 0.04 // 4% per point, up to 12% for value_rating=3
      }

      // Expert grade quality gates: Only A- or higher get bonus
      const aGradeBonus = calculateExpertGradeBonus(a as RecommendationComponent)
      const bGradeBonus = calculateExpertGradeBonus(b as RecommendationComponent)

      const aScore = aPriceFit * 0.6 + a.synergyScore * 0.4 + aGradeBonus
      const bScore = bPriceFit * 0.6 + b.synergyScore * 0.4 + bGradeBonus
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Parse driver type filter (v2 enhancement)
    let driverTypeFilter: string[] = []
    try {
      const driverTypeParam = searchParams.get('driverTypeFilter')
      if (driverTypeParam) {
        driverTypeFilter = JSON.parse(driverTypeParam)
      }
    } catch {
      driverTypeFilter = []
    }
    ...
    if (allComponentsData) {
      ...
      if (req.wantRecommendationsFor.headphones && componentsByCategory.headphones.length > 0) {
        ...
        results.headphones = filterAndScoreComponents(
          ...
          maxOptions,
          req.driverTypeFilter // v2: pass driver type filter
        )
      ...
      // Process AMPs if requested (with existing gear optimization)
      if (req.wantRecommendationsFor.amp && componentsByCategory.amps.length > 0) {
        ...
        let amps = filterAndScoreComponents(
          ...
          req.soundSignature,
          req.usageRanking[0] || req.usage,
          maxOptions * 2 // Get more candidates for optimization
        )

        // Optimize for existing headphones (v2 enhancement)
        if (req.existingGear?.specificModels?.headphones) {
          const headphoneName = req.existingGear.specificModels.headphones
          console.log('🎧 Optimizing amp recommendations for existing headphone:', headphoneName)

          // Try to find the user's headphone in database
          const { data: userHeadphone } = await supabaseServer
            .from('components')
            .select('impedance, name, brand, power_required_mw, voltage_required_v')
            .or(`name.ilike.%${headphoneName}%,brand.ilike.%${headphoneName}%`)
            .in('category', ['cans', 'iems'])
            .limit(1)

          if (userHeadphone && userHeadphone.length > 0) {
            const hp = userHeadphone[0]
            console.log('🎯 Found headphone:', hp.name, 'Impedance:', hp.impedance)

            // Boost amp scores based on headphone requirements
            amps = amps.map(amp => {
              // Assess headphone power needs
              const powerAssessment = assessAmplificationFromImpedance(
                hp.impedance,
                false,
                hp.name,
                hp.brand
              ...
              // Boost score for amps that match power requirements
              if (powerAssessment.difficulty === 'very_demanding' || powerAssessment.difficulty === 'demanding') {
                // Headphone needs serious power, prefer more expensive/powerful amps
                if (amp.avgPrice > 300) {
                  amp.synergyScore = Math.min(1, amp.synergyScore + 0.15)
                }
              } else if (powerAssessment.difficulty === 'easy') {
                // Headphone is easy to drive, cheaper amps are fine
                if (amp.avgPrice < 200) {
                  amp.synergyScore = Math.min(1, amp.synergyScore + 0.1)
                }
              }
            ...
            amps.sort((a, b) => {
              const aScore = a.avgPrice <= ampBudget ? a.synergyScore : a.synergyScore * 0.8
              const bScore = b.avgPrice <= ampBudget ? b.synergyScore : b.synergyScore * 0.8
              return bScore - aScore
            })
          }
        }

        results.amps = amps.slice(0, maxOptions)
